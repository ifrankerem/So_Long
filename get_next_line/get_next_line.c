...
/*   Updated: 2025/03/08 15:38:58 by iarslan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_free(char *buf, char *readed_line)
{
	...
	temp = gnl_strjoin(buf, readed_line);
	free(buf);
	return (temp);
}
...
char	*ft_next(char *buf)
{
	...
	newline_pos = gnl_strchr(buf, '\n');
	if (!newline_pos)
	{
		free(buf);
		return (NULL);
	}
	new_remainder = gnl_strdup(newline_pos + 1);
	free(buf);
...
char	*ft_extract_line(char *buf)
{
	...
	if (buf[copy_length] == '\n')
		copy_length++;
	line = gnl_calloc(copy_length + 1, sizeof(char));
	if (!line)
		return (NULL);
	gnl_memcpy(line, buf, copy_length);
	return (line);
}
...
char	*ft_read(int fd, char *buf)
{
	...
	if (!buf)
		buf = gnl_calloc(1, 1);
	readed_line = gnl_calloc(BUFFER_SIZE + 1, sizeof(char));
	if (!readed_line || !buf)
